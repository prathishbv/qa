...
def like_view(request, pk):
    ...
    if post.likes.filter(id=request.user.id).exists():
        ...
        liked = True
    return HttpResponseRedirect(reverse('backend:question-detail', args=[str(pk)]))
...
class CommentDetailView(CreateView):
    ...
    template_name = 'backend/question-detail.html'
    
    def form_valid(self, form):
        form.instance.question_id = self.kwargs['pk']
        return super().form_valid(form)
    success_url = reverse_lazy('backend:question-detail')

class AddCommentView(CreateView):
    model = Comment
    form_class = CommentForm
    
    template_name = 'backend/question-answer.html'

    def form_valid(self, form):
        form.instance.question_id = self.kwargs['pk']
        return super().form_valid(form)
    success_url = reverse_lazy('backend:question-lists')